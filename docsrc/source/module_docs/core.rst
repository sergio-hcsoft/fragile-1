API abstract classes
=====================

:class:`BaseSwarm`
^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseSwarm
   :members:

:class:`BaseWalkers`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseWalkers
   :members:

:class:`BaseEnvironment`
^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseEnvironment
   :members:

:class:`BaseModel`
^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseModel
   :members:

:class:`BaseStateTree`
^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseStateTree
   :members:

:class:`BaseCritic`
^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseCritic
   :members:

:class:`BaseWrapper`
^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.base_classes.BaseWrapper
   :members:

Swarm
=====

:class:`Swarm`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.swarm.Swarm
   :members:

Walkers
========

:class:`Walkers`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.walkers.Walkers
   :members:

Environment
===========

:class:`Environment` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.env.Environment
   :members:

:class:`DiscreteEnv`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.env.DiscreteEnv
   :members:

Model
======

:class:`Model` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.Model
   :members:

:class:`_DtModel` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models._DtModel
   :members:

:class:`ContinuousModel` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.ContinuousModel
   :members:

:class:`DiscreteModel` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.DiscreteModel
   :members:


:class:`DiscreteUniform`
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.DiscreteUniform
   :members:

:class:`BinarySwap`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.BinarySwap
   :members:

:class:`ContinuousUniform`
^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.models.ContinuousUniform
   :members:

States
======

:class:`States`
^^^^^^^^^^^^^^^^^^^^

.. autoclass:: fragile.core.states.States
   :members:

:class:`StatesEnv`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.states.StatesEnv
   :members:

:class:`StatesModel`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.states.StatesModel
   :members:

:class:`StatesWalkers`
^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.states.StatesWalkers
   :members:

DtSampler
=========

:class:`BaseDtSampler` (Abstract)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.dt_samplers.BaseDtSampler
   :members:

:class:`ConstantDt`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.dt_samplers.ConstantDt
   :members:

:class:`UniformDt`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.dt_samplers.UniformDt
   :members:

:class:`GaussianDt`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.dt_samplers.GaussianDt
   :members:

StateTree
=========

:class:`HistoryTree`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.tree.HistoryTree
   :members:

Bounds
======

:class:`Bounds`
^^^^^^^^^^^^^^^^^^^^
.. autoclass:: fragile.core.bounds.Bounds
   :members: