...
The following classes represent different aspects of modelling a given problem. They are state-less, and they write and save data to the :class:`States`.
...
* :model:`Model`: This implements the policy used to sample new actions that will be applied to the :class:`Environment`.

* :walkers:`Walkers`: This class takes care of the data processing necessary to implement the FractalAI algorithms.

All the data needed to make a :class:`Swarm` evolve is stored inside :class:`States`, and all the historical data generated is manged by the :class:`StateTree`.
The :class:`States` contain functionality for storing, accessing and cloning all the data of its respective classes.
...
* :env-st:`StatesEnv`: Contains the data provided by the :class:Environment. This data describes the problem being solved. It contains at least the following data:
    - ``states``: This data tracks the internal state of the :class:`Environment` simulation, and it is used to save and restore its state.
    - ``observs``: This is the data that corresponds to the observable representation of the :class:`Environment` state.
...
* :model-st:`StatesModel`: Contains the data provided by the :class:Model. This data describes the internal state of the :class:`Model`, and the actions applied to the :class:`Environment`.
    - ``actions``: It contains the actions that will be applied to the states.

* :walkers-st:`StatesWalkers`: This data describes the internal state of the :class:`Walkers`.
...
the different :class:`States` classes that will be read, and after the ``->`` are described the :class:`States` that each method modifies.
