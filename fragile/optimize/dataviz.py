...
class PlotSwarm(FunctionMapper):
    ...
    def stream_best(self):
        ...
        best_path = pd.DataFrame(
            {"x": [x], "y": [y] , "count": [self.walkers.n_iters],
    ...
    def stream_walkers(self):
        ...
        best_path = pd.DataFrame(
            {"x": [df.loc[0, "x"]], "y": [df.loc[0, "y"]], "count": [self.walkers.n_iters],
    ...
    def plot_best_path(self) -> hv.Scatter:
        ...
        best_curve_dmap = best_curve_dmap.opts(hv.opts.Curve(line_width=1, color='green', xlim=(
            self.env.bounds.low[0], self.env.bounds.high[0]), title="Evolution of best solution",
                                                             ylim=(self.env.bounds.low[1],
                                                                   self.env.bounds.high[1]), ))
    ...
    def plot_walker_path(self):
        ...
        walker_point_dmap = walker_point_dmap.opts(color='value', line_color='black', size=5,
                                                   xlim=(self.env.bounds.low[0],
                                                         self.env.bounds.high[0]),
                                                   ylim=(self.env.bounds.low[1],
                                                         self.env.bounds.high[1]),
                                                   padding=0.1,
    ...
    def plot_walkers(self) -> hv.Scatter:
        ...
        walkers_plot = walkers_plot.opts(tools=["hover"], title="Walkers",
                                         xlabel="first dimension of data",
                                         ylabel="second dimension of data",
                                         xlim=(
                                             self.env.bounds.low[0], self.env.bounds.high[0]),
                                         ylim=(
                                             self.env.bounds.low[1], self.env.bounds.high[1]),
                                         color="reward", size="virtual_reward",  # marker="dead",
                                         cmap="viridis"
                                         )
        return walkers_plot
